...
")
  ...
  if (dati.tipo_operazione === 'modifica' && dati.preventivo_originale_id) {
    ...
    if (existingRecords && existingRecords.length > 0) {
      // Prendi il primo (più recente) e aggiorna quello
      const firstRecord = existingRecords[0];
      
      // Se ci sono duplicati, elimina tutti tranne il primo
      if (existingRecords.length > 1) {
        ...
        await supabase
          ...
          .delete()
          .in("id", duplicateIds);
        console.log(`Eliminati ${duplicateIds.length} duplicati per modifica preventivo ${dati.preventivo_originale_id}`);
      }
  ...
  if (dati.numero && dati.anno && (dati.tipo_operazione === 'creazione' || dati.tipo_operazione === 'clonazione')) {
    // Prima cerca tutti i record con stesso numero, anno e tipo_operazione
    const { data: existingRecords, error: searchError } = await supabase
      .from("preventivi_cache")
      .select("id")
...
export async function pulisciDuplicatiCache() {
  try {
    // Carica tutti i record dalla cache
